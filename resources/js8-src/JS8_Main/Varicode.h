...
// wfweb vendor: added QMap / QPair / QSet / QList — upstream gets them
// via transitive Qt-internal includes that our minimal shim doesn't
// reproduce. Keeping it explicit here makes the surface auditable.
#include <QBitArray>
#include <QList>
#include <QMap>
#include <QPair>
...
// wfweb vendor: BuildMessageFramesThread (a QThread subclass that runs
// Varicode::buildMessageFrames in the background for JS8Call's GUI) was
// removed. The wfweb WASM bridge schedules its own work on the JS Worker
// side and calls the static buildMessageFrames() directly. Removing the
// QThread subclass also lets us drop QThread entirely from the shim
// surface that needs to actually inherit-from / override anything.
