...
// wfweb vendor: boost::math::ccmath::round (constexpr round) was added in
// Boost 1.79; the Emscripten port pins boost-headers to 1.75. We provide
// a tiny constexpr round in the JS8 namespace below and the
// `using boost::math::ccmath::round;` line was patched accordingly.
...
template <typename Mode> class DecodeMode {
    ...
    void baselinejs8(int const ia, int const ib) {
        ...
        // wfweb vendor: replaces `using boost::math::ccmath::round;`
        // (Emscripten ships Boost 1.75 which predates ccmath::round).
        // Constexpr round-half-away-from-zero on a double; only used
        // here in two compile-time evaluations of positive constants,
        // so the simple form is enough.
        constexpr auto round = [](double x) constexpr -> long long {
...
// DecoderImpl
//
// wfweb vendor: upstream wraps this class as `Worker::Impl`, where Worker is
// a QObject orchestrating a Qt thread + semaphore + signal emission. We want
// neither Qt threading nor signals/slots — the wfweb WASM bridge calls into
// the codec from a Web Worker via the C API in api/js8_wasm_api.cpp, doing
// its own threading on the JS side. So Worker is gone, and what was its
// nested Impl is hoisted to be the outer class here, renamed DecoderImpl.
/******************************************************************************/
...
namespace JS8 {
  ...
  public:
    ...
    explicit DecoderImpl(struct dec_data &data) : m_data(data) {}
    ...
    void operator()(::JS8::Event::Emitter emitEvent) {
        ...
        for (auto &entry : m_decodes) {
            if ((set & entry.mode) == entry.mode) {
                // wfweb vendor: upstream uses std::visit with a generic
                // lambda. libc++14 (the version that ships with emcc 3.1.6
                // on Debian) has a known return-type deduction bug for
                // std::visit when the lambda body invokes the alternative's
                // operator(). Manual std::get_if dispatch sidesteps it.
                auto &v = entry.decode;
                if      (auto* p = std::get_if<DecodeMode<ModeA>>(&v)) sum += (*p)(m_data, entry.kpos, entry.ksz, emitEvent);
                else if (auto* p = std::get_if<DecodeMode<ModeB>>(&v)) sum += (*p)(m_data, entry.kpos, entry.ksz, emitEvent);
                else if (auto* p = std::get_if<DecodeMode<ModeC>>(&v)) sum += (*p)(m_data, entry.kpos, entry.ksz, emitEvent);
                else if (auto* p = std::get_if<DecodeMode<ModeE>>(&v)) sum += (*p)(m_data, entry.kpos, entry.ksz, emitEvent);
                else if (auto* p = std::get_if<DecodeMode<ModeI>>(&v)) sum += (*p)(m_data, entry.kpos, entry.ksz, emitEvent);
            }
        }
...
//
// (Upstream's JS8::Decoder QObject + JS8.moc include lived here. Removed
// for the WASM port; see api/js8_wasm_api.cpp for the replacement entry
// points the JS Worker harness calls into.)
/******************************************************************************/
...
/******************************************************************************/
// wfweb WASM bridge — factory entry points so api/js8_wasm_api.cpp can
// construct and invoke DecoderImpl without needing its full definition
// at the API site (DecoderImpl is local to this TU).
/******************************************************************************/
