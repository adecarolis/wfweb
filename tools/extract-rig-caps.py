...
resources/web/civ/rig-caps.js — a global IIFE that publishes
...
RIGS_DIR = REPO / "rigs"
OUT = REPO / "resources/web/civ/rig-caps.js"
...
def main() -> int:
    rigs: list[tuple[int, str, dict, dict, dict, dict]] = []
    for p in sorted(RIGS_DIR.glob("*.rig")):
        ...
        rigs.append((civ, model, meters, cmds, caps, inputs))
    ...
    lines = [
        ...
        "//",
        "// Each entry: civAddr -> { model, caps, meters, cmds, inputs }",
    ...
    for civ, model, meters, cmds, caps, inputs in rigs:
        lines.append(
    ...
    for civ, model, meters, cmds, caps, inputs in rigs:
        ...
        meter_kinds = ",".join(sorted(meters.keys())) or "-"
        cmd_keys = ",".join(sorted(cmds.keys())) or "-"
        usb = inputs.get("USB", -1)
        usb_str = f"0x{usb:02x}" if usb >= 0 else "-"
        print(f"  0x{civ:02X}  {model:<18}{flag}  USB-reg={usb_str}")
    return 0
