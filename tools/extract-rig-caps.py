...
def main() -> int:
    ...
    for p in sorted(RIGS_DIR.glob("*.rig")):
        ...
        scope_edge_ranges = extract_num_name_list(props, "ScopeEdgeRanges")
        rigs.append((civ, model, meters, cmds, caps, inputs, preamps, attenuators, antennas, bands, scope_modes, scope_edge_ranges, mem_format))
    ...
    lines = [
        ...
        "//",
        "// Each entry: civAddr -> { model, caps, meters, cmds, inputs, preamps, attenuators, antennas, bands, scopeModes, scopeEdgeRanges, memFormat }",
        ...
        "//   scopeEdgeRanges: [{num, name, minFreq, maxFreq}, ...] — the frequency",
        "//           ranges the fixed-edge command (0x27 0x1E) addresses by number;",
        "//           empty = the SPA can't set Fixed-mode edges on this rig.",
    ...
    for civ, model, meters, cmds, caps, inputs, preamps, attenuators, antennas, bands, scope_modes, scope_edge_ranges, mem_format in rigs:
        lines.append(
            ...
            f"scopeEdgeRanges: {js_num_name_list(scope_edge_ranges)}, "
